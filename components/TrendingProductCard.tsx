  ...
  return (
    ...
    >
      <div className={`relative flex h-36 items-center justify-center ${product.imageColor}`}>
        ...
        <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-card/90 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide text-foreground shadow-sm">
          <TrendingUp className="h-3 w-3" />
          {product.trend}
        </span>
        <span className="absolute bottom-3 left-3 rounded-md bg-foreground/70 px-2 py-0.5 text-[10px] font-medium text-white">
          {product.category}
        </span>
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mb-4 space-y-1.5 text-xs text-muted-fg">
          ...
          <div className="flex items-center justify-between pt-1">
            <span className="flex items-center gap-1">
              <Star className="h-3.5 w-3.5 fill-muted-fg text-muted-fg" />
              {product.rating.toFixed(1)}
            </span>
            <span className="text-muted-fg">{product.inquiries} inquiries this week</span>
          </div>
        </div>
