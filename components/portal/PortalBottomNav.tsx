...
export default function PortalBottomNav({ items, accent = "buyer" }: PortalBottomNavProps) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-portal-border bg-card/95 px-1 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl sm:px-2 lg:hidden">
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`relative flex min-w-0 flex-1 flex-col items-center gap-0.5 px-0.5 py-2 text-[10px] font-medium transition-colors duration-200 sm:px-1 sm:py-2 ${
